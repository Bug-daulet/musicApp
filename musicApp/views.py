...
def index(request):
    ...
    #Display suggestions
    if not request.user.is_anonymous:
        liked_genres = list(Favourite.objects.filter(user=request.user).values('song__genre').distinct())
        genres = [i['song__genre'] for i in liked_genres]
        songs_suggested = list(Song.objects.filter(genre__in=genres).values('id'))[:5]
        sliced_ids = list(i['id'] for i in songs_suggested)
        indexpage_suggested_songs = Song.objects.filter(id__in=sliced_ids)
    else:
        indexpage_suggested_songs = []
    ...
    context = {
        ...
        'query_search': False,
        'suggested_songs': indexpage_suggested_songs,
    }
...
def suggested(request):
    if not request.user.is_anonymous:
        liked_genres = list(Favourite.objects.filter(user=request.user).values('song__genre').distinct())
        genres = [i['song__genre'] for i in liked_genres]
        songs_suggested = list(Song.objects.filter(genre__in=genres).values('id'))
        ids = list(i['id'] for i in songs_suggested)
        songs = Song.objects.filter(id__in=ids)
    else:
        songs = None

    context = {'songs': songs}
    return render(request, 'musicapp/suggestions.html', context=context)
...
def categories(request):
    cts = [i[1] for i in Song.Category_Choice]
    context = {}

    for i in cts:
        if i == 'Nocategory':
            continue
        songs = Song.objects.filter(category=i)[:5]
        context[i] = songs

    return render(request, 'musicapp/Categories.html', context=context)


def category_detail(request, cat):
    songs = Song.objects.filter(category=cat)
    category_name = -1

    for i in Song.Category_Choice:
        if i[1] == cat:
            category_name = i[0]
            break
    ...
    if category_name != -1:
        ...
        return render(request, 'musicapp/category_detail.html', context=context)
